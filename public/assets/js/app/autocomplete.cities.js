    ...
    beforeRender: function (container, suggestions) {
      ...
      if (typeof query !== "undefined" && typeof suggestions !== "undefined") {
        ...
        if (suggestions.length <= 0) {
          if (
            query.substring(0, stateOrRegionKeyword.length) ===
            stateOrRegionKeyword
          ) {
...
function showErrorModal(jqXHR, title) {
  ...
  var content =
    '<h4 class="m-b-20 font-weight-bold">' + errorText.errorFound + ":</h4>";
  content += "<code>" + message + "</code>";
  if (isLoggedAdmin) {
    if (
      message.indexOf("collation") !== -1 ||
      message.indexOf("COLLATION") !== -1
    ) {
      content += "<br><br>";
      content +=
        "The database server <strong>character set</strong> and <strong>collation</strong> are not properly configured. ";
      content +=
        'Please visit the "Admin panel → System → System Info" for more information.';
    }
  }
...
function hideNoSuggestionNotice(query, stateOrRegionKeyword) {
  if (
    query.substring(0, stateOrRegionKeyword.length) === stateOrRegionKeyword
  ) {
    var suggestionsEl = $(".autocomplete-suggestions");
