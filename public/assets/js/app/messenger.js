...
$(function () {
  ...
  $(document).on(
    "click",
    ".list-box-action a, #groupedAction a, .call-xhr-action a",
    function (e) {
      e.preventDefault();
      ...
      if (confirmation) {
        var currentPageFilter = getUrlParameter("filter");
        var url = $(this).attr("href");
...
function getThreads(url) {
  $("#listThreads").append(
    '<img class="loading-img" src="' + loadingImage + '" />'
  );
      ...
      if (clickedEl) {
        if (data.type === "markAsRead" || data.type === "markAsUnread") {
          if (currentPageFilter === "unread") {
            ...
            var isSeen = clickedEl.closest(".list-group-item").hasClass("seen");
            ...
            clickedEl
              .attr("title", titleIs)
              .attr("data-original-title", titleIs)
              .tooltip("hide");
            if (typeof data.baseUrl !== "undefined") {
              newActionUrl =
                data.baseUrl +
                "?type=" +
                (isSeen ? "markAsRead" : "markAsUnread");
        ...
        if (
          data.type === "markAsImportant" ||
          data.type === "markAsNotImportant"
        ) {
          if (currentPageFilter === "important") {
            clickedEl.tooltip("hide");
            getThreads(currentUrl);
          } else {
            var isImportant = clickedEl.hasClass("markAsNotImportant");
            ...
            if (typeof data.baseUrl !== "undefined") {
              newActionUrl =
                data.baseUrl +
                "?type=" +
                (isSeen ? "markAsImportant" : "markAsNotImportant");
              clickedEl.attr("href", newActionUrl);
            }
          }
        }
...
function markAllAsRead() {
  $.ajax({
    url: siteUrl + "/account/messages/mark-as-read",
    method: "POST",
    data: {
      _token: $("input[name=_token]").val(),
    },
